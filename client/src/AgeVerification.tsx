 ...
 * 
 * Features:
 * - Webcam-based verification for desktop users
 * - QR code for mobile verification (for users without webcams)
 */
...
type VerificationState = 
  ...
  | 'qr-mode';     // QR code mode for mobile verification
...
// Generate unique session ID for QR verification
...
const AgeVerification: React.FC<AgeVerificationProps> = ({ 
  ...
  // QR code mobile verification state
  const [sessionId] = useState<string>(() => generateSessionId());
  const [qrPolling, setQrPolling] = useState<boolean>(false);
  const [mobileStatus, setMobileStatus] = useState<string>('Waiting for mobile verification...');

  // Generate QR verification URL
  const qrVerificationUrl = `${getBaseUrl()}/verify-mobile?session=${sessionId}`;
    ...
    try {
      ...
      if (detections) {
        ...
        if (ageReadings.length >= 5 && !showCountdown) {
          ...
          if (avgAge >= minAge - AGE_TOLERANCE) { // Allow some margin for estimation accuracy
  ...
  // Poll for mobile verification status
  useEffect(() => {
    if (!qrPolling || state !== 'qr-mode') return;

    const pollInterval = setInterval(async () => {
      try {
        const response = await fetch(`/api/verify-age/status/${sessionId}`);
        const data = await response.json();
        
        if (data.verified === true) {
          setMobileStatus('Verification successful on mobile!');
          setState('success');
          setQrPolling(false);
          ...
          setMobileStatus('Waiting for mobile verification...');
        } else if (data.status === 'scanning') {
          setMobileStatus('QR code scanned! Complete verification on mobile...');
        } else if (data.verified === false) {
          setMobileStatus('Verification failed on mobile. Try again.');
          setQrPolling(false);
        }
      } catch (err) {
        // Server might not have the endpoint yet, continue polling
        console.log('Polling for mobile verification status...');
      }
    }, 2000);

    return () => clearInterval(pollInterval);
  }, [qrPolling, state, sessionId, onVerified]);
  ...
  // Switch to QR mode for mobile verification
  const handleSwitchToQR = () => {
    stopCamera();
    setState('qr-mode');
    setQrPolling(true);
    setMobileStatus('Scan the QR code with your mobile phone...');
    
    // Register session with server
    fetch('/api/verify-age/session', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ sessionId })
    }).catch(() => {
      // Server endpoint might not exist yet, continue anyway
      console.log('Session registered (or endpoint not available)');
    });
  };
      ...
      <div className="max-w-2xl w-full bg-neutral-800 border-2 border-blue-500/30 rounded-2xl p-8 shadow-2xl">
        ...
        {state === 'qr-mode' && (
          <div className="text-center">
            ...
            <div className="mb-6">
              <p className="text-xl text-white mb-2">📱 Scan with your mobile phone</p>
            ...
            <div className="bg-neutral-900/50 rounded-lg p-4 mb-6 text-left">
              <h4 className="text-sm font-semibold text-neutral-300 mb-2">📋 Instructions:</h4>
              <ol className="text-xs text-neutral-500 space-y-1 list-decimal list-inside">
                ...
                <li>Complete face verification on your phone</li>
                <li>This page will automatically update when done</li>
              </ol>
            </div>
            ...
            >
              ← Back to webcam verification
            </button>
          </div>
        )}

        {/* Video/Canvas Container (non-QR modes) */}
        {state !== 'qr-mode' && (
          <>
            ...
            <div className="flex flex-col gap-3">
              {state === 'ready' && (
                <>
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-neutral-700 hover:bg-neutral-600 text-white rounded-lg transition-colors flex items-center justify-center gap-2"
                  >
                    <span>📱</span>
                    <span>No webcam? Verify with mobile phone</span>
                  </button>
                </>
              )}
              ...
              {(state === 'failed' || state === 'error') && (
                <>
                  ...
                  {/* QR Code Option on failure */}
                  <button
                    onClick={handleSwitchToQR}
                    className="w-full py-3 bg-neutral-700 hover:bg-neutral-600 text-white rounded-lg transition-colors flex items-center justify-center gap-2"
                  >
                    <span>📱</span>
                    <span>Try with mobile phone instead</span>
                  </button>
                </>
              )}
